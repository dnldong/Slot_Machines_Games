...
import { createApp, ref, defineComponent, onUnmounted, onMounted, computed, watch } from 'vue';

// Default mock data with 5-digit codes for the slots
...
const HorseWatermark = {
  template: `
    <svg viewBox="0 0 500 500" class="w-full h-full opacity-30">
       ...
       <!-- Powerful Galloping Horse Silhouette -->
...
const HorseHerd = {
  template: `
    <div class="fixed top-1/2 left-0 w-full h-64 pointer-events-none z-40 transform -translate-y-1/2 overflow-hidden">
        ...
        <!-- Define the running horse shape once -->
        <svg class="hidden">
           <defs>
             <g id="running-horse">
...
const ApricotBranch = {
  template: `
    <svg viewBox="0 0 400 300" class="w-full h-full drop-shadow-2xl">
      <!-- Old Gnarly Branch -->
      <path d="M0,300 C50,250 100,280 150,220 C180,180 120,150 160,100 C200,50 280,80 350,20" 
            fill="none" stroke="#4a3728" stroke-width="12" stroke-linecap="round" />
      <path d="M150,220 C200,200 250,230 300,200" 
            fill="none" stroke="#4a3728" stroke-width="8" stroke-linecap="round" />
      <path d="M160,100 C200,120 230,90 260,110" 
            fill="none" stroke="#4a3728" stroke-width="6" stroke-linecap="round" />
      
      <!-- Detailed Yellow Apricot Flowers (Mai) -->
      <defs>
        <g id="mai-flower">
...
const PeachBranch = {
  template: `
    <svg viewBox="0 0 400 300" class="w-full h-full drop-shadow-2xl">
      <!-- Old Gnarly Branch (Mirrored structure of Mai) -->
      <g transform="scale(-1, 1) translate(-400, 0)">
        <path d="M0,300 C50,250 100,280 150,220 C180,180 120,150 160,100 C200,50 280,80 350,20" 
              fill="none" stroke="#3E2723" stroke-width="12" stroke-linecap="round" />
        <path d="M150,220 C200,200 250,230 300,200" 
              fill="none" stroke="#3E2723" stroke-width="8" stroke-linecap="round" />
        <path d="M160,100 C200,120 230,90 260,110" 
              fill="none" stroke="#3E2723" stroke-width="6" stroke-linecap="round" />
        
        <!-- Detailed Pink Peach Flowers (Dao) -->
        <defs>
...
class FireworkSystem {
  ...
  particles: { x: number; y: number; vx: number; vy: number; alpha: number; color: string; decay: number }[];
...
const App = defineComponent({
  ...
  setup() {
    ...
    const winnersHistory = ref<any[]>([]);
    const reels = ref(['2', '0', '2', '6', '🐎']); // Initial state with Horse
    ...
    // Audio/Effects
    const fireworkSystem = ref<FireworkSystem | null>(null);
    ...
    onMounted(() => {
      ...
      if (canvasRef.value) {
        ...
        // Start fireworks and horses on mount
        fireworkSystem.value.start();
        
        // Stop fireworks and remove horses after a few seconds
        setTimeout(() => {
    ...
    const spin = async () => {
      ...
      // Use the first winner's code for visual animation target
      const targetCode = selected[0].code.split('');

      // 2. Spinning Phase (3 seconds)
      // We rely on CSS animation 'animate-slot-scroll' triggered by spinningReels=true
      await new Promise(resolve => setTimeout(resolve, 3000));

      // 3. Stop reels (simultaneously with effect)
      // Update DOM to target number and remove spin class
      for (let i = 0; i < 5; i++) {
        reels.value[i] = targetCode[i];
        spinningReels.value[i] = false;
        stoppingReels.value[i] = true; // Trigger bounce animation
      }
      
      // Clear bounce animation state after animation finishes
      setTimeout(() => {
          stoppingReels.value = [false, false, false, false, false];
      }, 600);
    ...
    const confirmReward = () => {
      if (currentWinners.value.length > 0) {
        ...
        // Adjust spin count if list is now smaller
      ...
      <div v-if="showSettings" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div class="bg-white rounded-xl w-full max-w-4xl h-[90vh] p-0 shadow-2xl flex flex-col overflow-hidden">
          ...
          <div class="p-4 md:p-6 bg-gray-50 flex-1 overflow-hidden flex flex-col">
             ...
             <!-- Expanded textarea height and width -->
             <textarea 
